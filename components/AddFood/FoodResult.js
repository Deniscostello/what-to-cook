...
function FoodResult(props) {
    ...
    if(props.response) {
        output = !props.response.recognized ?<div className={classes.output}> <p className={classes.output}>Cannot recognize food item!</p>  <input  type='text' placeholder='Enter Food Name' required id='name' ref={nameInputRef}></input><button onClick={ () => setCorrectName((nameInputRef.current.value))}>Submit text</button></div> 
